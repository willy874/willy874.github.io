...
- [x] **description 重寫**:430 篇全部改寫完畢,不再有任何一篇是內文硬切。
      每篇都是能離開文章單獨讀懂的完整句子,長度中位數 65 字(規範 60～100)
- [x] 9 篇正文完全空白(只有標題)的文章改為 `draft: true`,不再進 build / sitemap / RSS。
      清單見下方「近重複與殘稿」;內容一個字都沒動,想恢復只要把 draft 改回 false
- [ ] 近重複與殘稿的處理決策(需要你決定,我只做了最低限度的止血):
      已改 `draft: true` 的 9 篇空白文章 —— 讀書的方法03福爾摩斯圖像記憶法、61.8的留白、
      憑什麼說你是最好的？、投資先把自己的錢判無期徒刑、還做的不好沒關係但是要做要失敗、
      減少會議提高生產力？、貨幣的演進、［議題討論］亞尼克YTM販售機、［議題討論］高齡社會的發展。
      要補寫內文還是直接刪掉,你決定。
      另外還有 10 篇正文 13～59 字的殘稿仍是 published 狀態,列表用
      `find src/content/posts -name '*.md' | xargs wc -c | sort -n | head -20` 可以撈出來:
