...
上一篇寫 [Refresh Token](/posts/工程實務/refresh-token/)，反應意外地熱烈。
...
但只要稍微碰過資安你就知道，localStorage 根本是 XSS 的提款機。頁面上隨便一段被注入的 JS，一行 `localStorage.getItem` 就把你的 token 整包端走。這件事的前提我在[資安漏洞遠比你想像的還要多](/posts/工程實務/資安漏洞遠比你想像的還要多/)講過：前端沒有秘密，只要東西在前端，就有辦法被拿走。
...
而這其實也是我在[你可能不需要 SSR](/posts/工程實務/你可能不需要-ssr/) 裡想講的同一件事：SSR 帶來的每一個好處，背後都有一張帳單。認證這張，開得特別大。
...
說白了，這整件事的心態跟[零信任](/posts/工程實務/零信任/)是同一套：與其賭那些狀況不會同時發生，不如假設它們一定會發生，然後把爆炸半徑縮到最小。
